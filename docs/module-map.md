...
| 14 | `core/LauncherSetup.lua` | `NS.Launcher`: the one LibDataBroker object, its icon, its rung-(b) left click, the right click that always opens the panel, and the answers the library's status tooltip reads (enabled, locked, version, left-click hint) (launcher-§1/§2) | conventional: every seam is reached at click or `Register` time, and `Register` runs in `addon:OnEnable` |
